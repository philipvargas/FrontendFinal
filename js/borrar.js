...
function mostrarEliminar(idBike, infBike) {
    ...
    $("#listado").hide();
    $("#nuevoRegistro").hide();
    $("#titleIdDelete").html("Desea eliminar el bote con el id: " + idBike + " ?");
    $("#idDelete").val(idBike);
...
function borrarRegistro(llaveRegistro) {
    ...
    $.ajax({
        ...
        error: function(xhr, status) {
            $("#mensajes").html("El Bote que intenta eliminar tiene información relacionada, por favor verifique...");
            $("#mensajes").show(1000);
        }
    });
}