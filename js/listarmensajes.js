...
function listar() {
    $.ajax({
        ...
        url: "http://129.151.125.65:8081/api/Message/all",
...
function listarRespuesta(items) {
    ...
    var tabla = `<table border="1" class="table table-success table-striped">
                  <tr>
                    <th>Cliente</th>
                    <th>Texto</th>
    ...
    for (var i = 0; i < items.length; i++) {
        tabla += `<tr>
                   ...
                   <td><button class="botonEditar" onclick=editarRegistro(${items[i].idMessage})>Editar</button></td>
                   <td><button class="botonBorrar" onclick="mostrarEliminar(${items[i].idMessage},'${items[i].messageText}')">Borrar</button></td>
                   </tr>`;
    }
