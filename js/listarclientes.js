...
function listarRespuesta(items) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        tabla += `<tr>
                   ...
                   <td>${items[i].email}</td>
                   <td>${items[i].age}</td>
                   <td><button class="botonEditar" onclick="editarRegistro(${items[i].idClient})">Editar</button></td>
                   <td><button class="botonBorrar" onclick="mostrarEliminar(${items[i].idClient},'${texto}')">Borrar</button></td>
                   </tr>`;
    }
